...
Ok, so we have observed the star ratings and price range interaction. What of restaurant reviews? What types of restaurant exist, and which restaurants get the best reviews? The [Yelp developer documentation][1] is invaluable in this case as it contains a breakdown of the [category types][2] of restaurant listed by yelp. There are rather alot...
...
yelpBusinessData$city

# user_ids <- yelpReviewData %>% filter( business_id %in% lasVegasRestaurants$business_id )
# str(user_ids)
# userDetails <- yelpUserData %>% filter(user_id %in% user_ids$user_id)
# userDetails$user_id
