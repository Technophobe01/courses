...
In the context of Las Vegas we determined that **'Buffet'** restaurants recieve the highest review count by Star volume. Note that this also hilight a basic strength and weakness in the Yelp review system... A review is defined by the context of the review attributes of the observer... thus we get very different answers based on what we value... for example is we order by review volume, star rating and price we get one answer, if we order by price, star rating then review volume we get another...

```{r BusinessByReviewStar}

BusinessByReviewStar <- lasVegasRestaurants %>% arrange(desc(review_count), desc(stars), desc(attributes.Price_Range))  
...
# 
# Do a keep cleanup of categories...
d <- head(BusinessByReviewStar,10)
d$categories
d$categories[[1]] <- d$categories[[1]][c(-1,-2,-4)]
d$categories[[2]] <- d$categories[[2]][c(-2)]
d$categories[[3]] <- d$categories[[3]][c(-2)]
d$categories[[4]] <- d$categories[[4]][c(-2)]
d$categories[[5]] <- d$categories[[5]][c(-2)]
d$categories[[6]] <- d$categories[[6]][c(-2)]
d$categories[[7]] <- d$categories[[7]][c(-1,-3)]
d$categories[[8]] <- d$categories[[8]][c(-2,-3)]
d$categories[[9]] <- d$categories[[9]][c(-2)]
d$categories[[10]] <- d$categories[[10]][c(-1,-3)]
d$categories
d$categories <- unlist(d$categories)

# Re-arrnage in preperation for display
d <- d %>% arrange(desc(review_count))

#  Calculate the midpoints of the bars.
#  Ref: http://stackoverflow.com/questions/6644997
#  
d <- ddply(d, .(categories), transform, pos = cumsum( review_count ) - (0.5 * review_count))
...
gp <- gp + ggtitle("Top Ten Restaurant Type by Review Incidence \nby Star Volume - Las Vegas\n")
print(gp)
