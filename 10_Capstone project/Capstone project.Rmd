...
``` {r InitialAnalysis}
# Use src_mysql to connect to an existing mysql or mariadb database, and tbl to
# connect to tables within that database. If you are running a local mysqlql
# database, leave all parameters set as their defaults to connect. If you're
# connecting to a remote database, ask your database administrator for the
# values of these variables.
# 
# User name and password. Rather than supplying a username and password here,
# it's better to save them in my.cnf, as described in MySQL. # In that case,
# supply NULL to both user and password.

conDB <- src_mysql(dbname = 'Capstone', user = NULL, password = NULL)

# Select the yelpBusinessData$city data from the sql database...
MyData <- as.data.frame(conDB %>% tbl("yelpBusinessData") %>% select(city))
head(MyData,5)
tail(MyData,5)
