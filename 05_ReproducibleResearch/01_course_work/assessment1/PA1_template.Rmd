...
requiredPackages <- c("ggplot2", "scales", "lubridate", "gridExtra")
...
avgSteps$interval <- sprintf("%0d", avgSteps$interval)
...
To determien this we determine the max value in the avgSteps and use that as the index into avgSteps to return the correct time index. The index time matches to the time series diagram. (**Useful Self check**)
...
The total number of NA values is **2304**, we caculate this by summing the number of values that are NA in the **activityData$steps** set. Here we use the complete.case() function to return a logical vector indicating which cases are complete, i.e., which have have missing values. We negate the result to determine this.
...
The replacement of the the NA values in the activity data set with the averaged data values has the impact of increasesing the peak from 10 to 18 observations. This arguably does not impact the overall data (at least in my opinion).
...
gp <- ggplot(impsteps, aes(x = interval, 
                           ...
                           group = weekday, 
                           color = weekday)) 
# gp <- gp + theme_solarized(light = FALSE)
gp <- gp + geom_line() 
gp <- gp + facet_grid( weekday ~ .) 
gp <- gp + theme(axis.text.x=element_text(angle=270,hjust=1,vjust=0.5, size = 10))
gp <- gp + scale_x_datetime(breaks = date_breaks("30 mins"),
                            labels = date_format("%H%M"),
                            limits = c(impsteps$interval[12], impsteps$interval[286-10]))
gp <- gp + ylab("Average steps")  
gp <- gp + xlab("5-minute Time Intervals (Labeled in chunks of 30-minutes)") 
