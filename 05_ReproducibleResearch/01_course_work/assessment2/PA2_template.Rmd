...
The final component of the dat processing is to create a tidy data set for future use that incorporates the changes we have applied to the data.
...
```{r create eventTypeImpactSummary }
eventTypeImpactSummary <- stormData %>%
  group_by(eventType) %>%
...
```{r populationHealth, comment=NA, results='asis'}

print(xtable(head(eventTypeImpactSummary, n = 10),
...
```{r plot_populationHealth, comment=NA, results='asis'}
...
# stormDataDamage <- filter(stormData, 
#                           stormData$PROPDMGEXP== "B" | 
#                           stormData$PROPDMGEXP== "M" |
#                           stormData$PROPDMGEXP== "K" )
# stormDataDamage %>% 
#         arrange(eventBeginYear) %>% 
#         mutate(totalPropDamage = (PROPDMG * billion)) %>%
#         mutate(totalCropDamage = (CROPDMG * billion)) %>%
#         mutate(totalDamage = ((PROPDMG * billion) + (CROPDMG * billion))) %>%
#         select(eventBeginYear,eventType, PROPDMG, PROPDMGEXP, CROPDMG, CROPDMGEXP, totalPropDamage, totalCropDamage, totalDamage) 
```
# References
