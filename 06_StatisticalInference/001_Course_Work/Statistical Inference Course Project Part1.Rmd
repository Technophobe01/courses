...
```{r SampleMean}
simulatonMeans <- rowMeans(simulationData)
x_bars <- apply(simulationData, 1, mean)

sim <- data.frame(simulationData) %>% mutate(row.Mean = rowMeans(.[]))

data.frame(simulationData) %>% mutate(test1) 
compare(data.frame(simulationData),t)
