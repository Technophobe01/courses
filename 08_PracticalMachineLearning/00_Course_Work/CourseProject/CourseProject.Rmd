...
The purpose of this document is to develop and answer the [Coursera Peer Assessment for the Practical Machine Learning Course][0].
...
# Load the baseData in preperation for review, cleanup and subsequent manipulation...

trainBaseData <- read.csv(trainDataPath)
testBaseData <- read.csv(testDataPath)
...
The our first step is to download and load the data from `pml-training.csv` and `pml-testing.csv` into R. We note that `trainBaseData` has `r dim(trainBaseData)` objects and variables, whilst `testBaseData` has `r dim(testBaseData)` objects and variables.
...
# First step - strip out the columns that have NA data
# 

testBaseData <- testBaseData[, colSums(is.na(testBaseData)) != nrow(testBaseData)]
dim(testBaseData)

trainBaseData <- trainBaseData[, colSums(is.na(trainBaseData)) != nrow(trainBaseData)]
dim(trainBaseData)

# Woops - the above will not work as their is messy data in the trainBaseData...
